...
- type (тип): `f` и `F` указывают, что входные данные являются числом с плавающей запятой и должны быть отображены как таковые, `e` и `E` указывают, что входные данные являются числом с плавающей запятой и должны быть отображены как показатель степени (частично сломано), `g` и `G` указывают, что входные данные являются числом с плавающей запятой—отображатель будет использовать десятичную запись для малых значений и экспоненциальную запись для больших значений, `d` указывает, что входные данные являются целым числом и должны быть отображены в десятичной системе счисления, `x` и `X` требуют целого числа и отобразят его как шестнадцатеричные цифры, `o` требует целого числа и отобразит его как восьмеричные цифры, `b` требует целого числа и отобразит его как двоичные цифры, `s` требует строку (почти никогда не используется).
  ...
  > **Примечание**
  >
  > Когда числовой тип может отображать алфавитные символы, например, шестнадцатеричные строки или специальные значения вроде `infinity`, строчная версия типа强制使用 строчные буквы, а прописная версия强制使用 прописные буквы.
...
- строки формата по умолчанию используют строчные буквы для шестнадцатеричных цифр и `e` в показателях степени. Используйте прописной тип, чтобы强制使用 прописные шестнадцатеричные цифры и прописную `E` в показателях степени.
...
Элемент можно добавить в конец массива с помощью оператора推送 `<<`.
...
Когда идентификатор является单一ным подчёркиванием, он игнорируется.
...
И ключ, и значение можно игнорировать, используя单一ное подчёркивание в качестве идентификатора.
...
`$if debug` — специальная опция,类似но `$if windows` или `$if x32`, она включена, если программа
...
производительность и уменьшить размер результирующего исполняемого файла, но增加了 сложность процесса сборки.

**Когда использовать**

- Для приложений, критичных к производительности, где增加了 сложность сборки оправдана.

**Когда избегать**

- Для небольших, кратковременных или быстро меняющихся проектов, где增加了 сложность сборки не
оправдана.
...
- `-g` - создает менее оптимизированный исполняемый файл с большим количеством отладочной информации.
  V будет强迫овать номера строк из .v файлов в трассировках стека, которые
...
существует, она рекомендуется только для特定ных сценариев использования, таких как скрипты, которые будут помещены в путь и
